...
export default function Form1099sPage() {
  ...
  return (
    <div className="root">
      ...
      <main className="main">
        <h1 className="title">Form 1099-S <span className="ref">Proceeds from Real Estate Transactions</span></h1>

        <div className="card">
          <h2>Filer Information</h2>
          <div className="row"><TF label="Filer name (attorney/title company) *" fkey="payerName" fields={fields} set={set} full /></div>
          <div className="row"><EINField label="Filer EIN" value={fields.ein||""} onChange={(v:string)=>set("ein",v)} /><TF label="Account number" fkey="accountNumber" fields={fields} set={set} /></div>
        </div>
        <div className="card">
          <h2>Transaction Details</h2>
          <div className="row">
            <TF label="Date of closing (MM/DD/YYYY)" fkey="closingDate" fields={fields} set={set} />
            <MF label="Box 2 — Gross proceeds *" fkey="box2" fields={fields} set={set} />
          </div>
          <div className="row">
            <TF label="Box 3 — Address/description of property" fkey="box3" fields={fields} set={set} full />
          </div>
          <div className="row">
            <CB label="Box 4 — Transferor received or will receive property or services" fkey="box4" fields={fields} set={set} />
            <CB label="Box 5 — Buyer's part of real estate tax" fkey="box5" fields={fields} set={set} />
          </div>
          <div className="row">
            <MF label="Original purchase price (cost basis)" fkey="costBasis" fields={fields} set={set} />
            <MF label="Selling expenses (commissions, fees)" fkey="sellingExpenses" fields={fields} set={set} />
          </div>
          {(() => {
            const proceeds = parseFloat(fields.box2||"0")
            const basis = parseFloat(fields.costBasis||"0")
            const expenses = parseFloat(fields.sellingExpenses||"0")
            const gain = proceeds - basis - expenses
            return proceeds > 0 ? (
              <div className="calc-box">
                <div className="calc-row"><span>Gross proceeds</span><span>${proceeds.toLocaleString("en-US",{minimumFractionDigits:2})}</span></div>
                <div className="calc-row"><span>Cost basis</span><span>-${basis.toLocaleString("en-US",{minimumFractionDigits:2})}</span></div>
                <div className="calc-row"><span>Selling expenses</span><span>-${expenses.toLocaleString("en-US",{minimumFractionDigits:2})}</span></div>
                <div className="calc-row total" style={{color:gain>=0?"#dc2626":"#16a34a"}}><span>{gain>=0?"Capital gain":"Capital loss"}</span><span>${Math.abs(gain).toLocaleString("en-US",{minimumFractionDigits:2})}</span></div>
              </div>
            ) : null
          })()}
        </div>
