...
export default function Form8960Page() {
  ...
  return (
    <div className="root">
      ...
      <main className="main">
        <h1 className="title">Form 8960 <span className="ref">Net Investment Income Tax (3.8%)</span></h1>

        <div className="card">
          <h2>Net Investment Income</h2>
          <p className="note">The 3.8% NIIT applies to net investment income if your MAGI exceeds $200,000 (single) or $250,000 (MFJ).</p>
          <div className="row">
            <MF label="Taxable interest" fkey="interest" fields={fields} set={set} />
            <MF label="Ordinary dividends" fkey="dividends" fields={fields} set={set} />
            <MF label="Net capital gains" fkey="capitalGains" fields={fields} set={set} />
          </div>
          <div className="row">
            <MF label="Rental income (net of expenses)" fkey="rentalIncome" fields={fields} set={set} />
            <MF label="Passive activity income" fkey="passiveIncome" fields={fields} set={set} />
            <MF label="Other investment income" fkey="otherIncome" fields={fields} set={set} />
          </div>
          <div className="row">
            <MF label="Investment expenses (deductible)" fkey="investExpenses" fields={fields} set={set} />
          </div>
          <div className="row">
            <MF label="Modified AGI (MAGI)" fkey="magi" fields={fields} set={set} />
        ...
        <div className="card">
          <h2>NIIT Calculation</h2>
          {(() => {
            const interest = parseFloat(fields.interest||"0")
            const dividends = parseFloat(fields.dividends||"0")
            const capGains = parseFloat(fields.capitalGains||"0")
            const rental = parseFloat(fields.rentalIncome||"0")
            const passive = parseFloat(fields.passiveIncome||"0")
            const other = parseFloat(fields.otherIncome||"0")
            const expenses = parseFloat(fields.investExpenses||"0")
            const magi = parseFloat(fields.magi||"0")
            const fs = fields.filingStatus||"SINGLE"
            const threshold = fs === "MARRIED_FILING_JOINTLY" ? 250000 : fs === "MARRIED_FILING_SEPARATELY" ? 125000 : 200000
            const nii = Math.max(0, interest + dividends + capGains + rental + passive + other - expenses)
            const magiExcess = Math.max(0, magi - threshold)
            const niitBase = Math.min(nii, magiExcess)
            const niit = niitBase * 0.038
            return (
              <div className="calc-box">
                <div className="calc-row"><span>Total net investment income</span><span>${nii.toFixed(2)}</span></div>
                <div className="calc-row"><span>MAGI over threshold (${threshold.toLocaleString()})</span><span>${magiExcess.toFixed(2)}</span></div>
                <div className="calc-row"><span>Lesser amount subject to NIIT</span><span>${niitBase.toFixed(2)}</span></div>
                <div className="calc-row total" style={{color:niit>0?"#dc2626":"#16a34a"}}><span>Net Investment Income Tax (3.8%)</span><span>${niit.toFixed(2)}</span></div>
              </div>
            )
          })()}
        </div>
